_addon.author   = 'Kaotic, rewritten by Otamarai'
_addon.version  = '2.05'
...
function findWaitPosition()
	...
	wait_x = quetz_x - math.random(1,6)
	wait_y = quetz_y + math.random(0,5)
end
...
function fight()
	...
	if isBuffActive(603) then	
		...
		if player.status == 0 and isBuffActive(603) and fighting then			--If not engaged then engage
			...
			delay = 1
		elseif math.sqrt(quetz.distance) > 5 and player.status == 1 and fighting then		--Turn and run to quetz
		...
		elseif math.sqrt(quetz.distance) <= 7 and player.status == 1 and fighting then		--Summon trusts when they die
			windower.ffxi.run(false)
			if summonTrust() ~= false and not isBuffActive(6) and not isBuffActive(2) and not partymembers then
				windower.send_command('input /ma "'..summonTrust()..'" <me>')
				windower.send_command('@wait 3;input /ra <t>;')
			end
			delay = 3
		end
...
--]]
			windower.send_command('input //quetz stop')
			windower.send_command('input /ma warp <me>')
			coroutine.sleep(5)
			windower.send_command('input //azi start')
...
windower.register_event('incoming chunk',function(id,data,modified,injected,blocked)
	...
	elseif id == 0x036 and not isBuffActive(603) and pause == "off" then
		local parse = packets.parse('incoming', data)
