_addon.author   = 'Kaotic, rewritten by Otamarai, modified for AziDahaka by Wealer/eRUPT'
_addon.version  = '2.1'
_addon.commands = {'Azi'}
...
--			affi = windower.ffxi.get_mob_by_name('Affi')
		...
		elseif zone ~= 'Escha - Zi\'Tah' and zone ~= 'Qufim Island' then
			if find_hp() then
				windower.send_command('sw hp w qufim')
			else
...
function enterArena()
	afficached = {}
	afficached.x = -356.356
	afficached.y = -170.773
	local me = windower.ffxi.get_mob_by_target('me')
...
function findWaitPosition()
	...
	wait_x = azi_x - math.random(1,4)
	wait_y = azi_y + math.random(1,5)
end
...
function warp_out()
	windower.send_command('input //azi stop')
--	windower.send_command('input /ma warp <me>')
	windower.send_command('input //multi warp')
	coroutine.sleep(45)
	windower.send_command('input //multi warp')
	windower.send_command('input //naga start')
